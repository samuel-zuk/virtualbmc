...
class BMCConfigTestCase(base.TestCase):
    ...
    def setUp(self):
        super().setUp()
        app_conf_patcher = mock.patch(
            'virtualbmc.conf.CONF',
    ...
    def test_new(self, mock_libvirt_register_opts):
        bmc_type = 'libvirt'
        name = 'test-bmc'
        ...
        mock_libvirt_register_opts.assert_called_once()

    @mock.patch('virtualbmc.conf.bmc.libvirt.register_opts')
    def test_new_with_args(self, mock_libvirt_register_opts):
        bmc_type = 'libvirt'
        name = 'test-bmc'
        host_ip = '10.0.0.1'
        port = 1234
        ...
        self.CONF.new(bmc_type, name,
                      host_ip=host_ip, port=port, enabled=enabled)
